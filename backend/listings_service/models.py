...
class Listing(models.Model):
    ...
    locationdesc = models.TextField(validators=[MinLengthValidator(10)])
    addresstext = models.CharField(max_length=50, validators=[MinLengthValidator(5)])
    propertytype = models.CharField(max_length=20, choices=PropertyType.choices)
    pricepernight = models.IntegerField(validators=[MinValueValidator(0)])
    maxguests = models.IntegerField(validators=[MinValueValidator(1)])

    class Meta:
        db_table = 'accomodation'
        constraints = [
            models.CheckConstraint(condition=Q(bedrooms__gte=1), name='listing_bedrooms_gte_1'),
            models.CheckConstraint(condition=Q(bathrooms__gte=1), name='listing_bathrooms_gte_1'),
            models.CheckConstraint(condition=Q(maxguests__gte=1), name='listing_maxguests_gte_1'),
            models.CheckConstraint(condition=Q(pricepernight__gte=0), name='listing_pricepernight_gte_0'),
...
# --- AQUÍ ESTABA EL DETALLE: ListingImage debe estar afuera, no dentro de Listing ---

class ListingImage(models.Model):
    def upload_path(instance, filename):
        return os.path.join('listings', str(instance.listing.pk), filename)
